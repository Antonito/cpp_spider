...
namespace spider
{
  namespace client
  {
	...
	return true;
      ...
      CGEventRef SpiderPlugin::treatMouseEvent(CGEventTapProxy proxy, CGEventType type,
          CGEventRef ref, void *refcon)
      {
        //treat event here
      }
      ...
      void SpiderPlugin::mouseThread()
      {
        ...
        if (!tap) {
          //TODO: rm
          std::cout << "Error creation hookMouse (need root)" << std::endl;
          return ;
        }
      ...
      CGEventRef SpiderPlugin::treatKeyboardEvent(CGEventTapProxy proxy, CGEventType type,
          CGEventRef eventRef, void *refcon)
      {
        //treat event here
        CGKeyCode key = CGEventGetIntegerValueField(eventRef, kCGKeyboardEventKeycode);
        //time_t now; time(&now); //get Time here
      ...
      void SpiderPlugin::keyboardThread()
      {
        ...
        if (!tap) {
          //TODO: rm
          std::cout << "Error creation hookKeyboard (need root)" << std::endl;
          return ;
        }
        CFRunLoopSourceRef runloop = CFMachPortCreateRunLoopSource(kCFAllocatorDefault, tap, 0);
        m_osxKeyboardLoop = CFRunLoopGetCurrent();
